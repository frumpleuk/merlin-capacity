...
// Bounds one invocation; a backlog drains over several nights. Steady state only
// ever has one day to move, so 3 is two nights of slack. It was briefly 10 to
// clear the 57-day backlog this job inherited (nothing archived queue days before
// it existed); that is drained, so it is back to the bound that fails softly.
const MAX_QUEUE_DAYS_PER_RUN = 3;
