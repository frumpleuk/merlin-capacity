...
export default {
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    // Menu-board photos (contrib/menus → R2 via scripts/menus/publish.mjs).
    // The web copies are immutable: a board that changes is a new photo on a
    // new date, under a new key. So cache them hard, at the edge and in the
    // browser — they're the heaviest thing this worker serves.
    if (url.pathname.startsWith("/menus/") && url.pathname.endsWith(".jpg")) {
      const obj = await env.BUCKET.get(url.pathname.slice(1));
      if (!obj) return new Response("not found", { status: 404 });
      return new Response(obj.body, {
        headers: {
